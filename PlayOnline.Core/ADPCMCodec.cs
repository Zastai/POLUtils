...
namespace PlayOnline.Core.Audio {
  ...
  internal class ADPCMCodec {
    ...
    public ADPCMCodec(int Channels, int BlockSize) {
      this.Reset(Channels, BlockSize);
    }
    ...
    static readonly int[] Filter0 = new int[] { 0x0000, 0x00F0,  0x01CC,  0x0188,  0x01E8 };
    static readonly int[] Filter1 = new int[] { 0x0000, 0x0000, -0x00D0, -0x00DC, -0x00F0 };
        ...
        if (Index < 5) {
          ...
          byte SampleByte = In[BaseIndex + Sample + 1];
            ...
            int TempValue = (Value << Scale);
              TempValue += ((this.DecoderState[Channel * 2 + 0] * ADPCMCodec.Filter0[Index] + this.DecoderState[Channel * 2 + 1] * ADPCMCodec.Filter1[Index]) / 256);
              this.DecoderState[Channel * 2 + 1] = this.DecoderState[Channel * 2 + 0];
              this.DecoderState[Channel * 2 + 0] = this.Round(TempValue);
              Out[((2 * Sample + Nibble) * this.Channels + Channel) * 2 + 0] = (byte) ((this.DecoderState[Channel * 2 + 0] >> 0) & 0xff);
              Out[((2 * Sample + Nibble) * this.Channels + Channel) * 2 + 1] = (byte) ((this.DecoderState[Channel * 2 + 0] >> 8) & 0xff);
            }
          }
        }
      }
