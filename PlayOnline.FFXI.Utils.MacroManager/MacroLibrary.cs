...
namespace PlayOnline.FFXI {
  ...
  public class MacroLibrary : SpecialMacroFolder {
    ...
    public override bool CanSave { get { return (this.FileName_ != null); } }

    private MacroLibrary() : base("Macro Library") { }

    public static MacroFolder Load() {
    MacroLibrary ML = new MacroLibrary();
      {
      string SettingsDir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), Path.Combine("Pebbles", "POLUtils"));
