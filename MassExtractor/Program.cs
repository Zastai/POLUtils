  ...
  internal class Program {
...
#if PrePatchExtractor

    private static void ExtractFile(string ROMFile, string OutputFile) {
      Console.ForegroundColor = ConsoleColor.White;
      Console.Write("Extracting {0}...", Path.GetFileName(OutputFile));
	...
	string[] Args = Environment.GetCommandLineArgs();
	  ...
	  if (!Directory.Exists(OutputFolder)) {
	    Console.ForegroundColor = ConsoleColor.White;
	    Console.WriteLine("Creating {0}...", OutputFolder);
	    Directory.CreateDirectory(OutputFolder);
	  }
	}
      DateTime ExtractionStart = DateTime.Now;
	Console.ForegroundColor = ConsoleColor.Yellow;
	Console.WriteLine(String.Format("Extraction started at {0:G}", ExtractionStart));
	...
	Console.ForegroundColor = ConsoleColor.Yellow;
	Console.WriteLine(String.Format("Extraction ended at {0:G}", ExtractionEnd));
	Console.WriteLine(String.Format("Elapsed time: {0:G}", ExtractionEnd - ExtractionStart));
	return 0;
      }
#if !DEBUG
      catch (Exception E) {
