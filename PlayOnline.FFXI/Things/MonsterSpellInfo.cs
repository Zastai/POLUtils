...
namespace PlayOnline.FFXI.Things {
  ...
  public sealed class MonsterSpellInfo : Thing {

    public MonsterSpellInfo() {
      // Clear fields
      this.Clear();
    }

    public override string ToString() {
      return String.Format("Spell #{0}", this.Index_);
    }
    ...
    public override List<string> GetAllFields() {
      return MonsterSpellInfo.AllFields;
    }
    ...
    public bool Read(BinaryReader BR) {
      ...
      byte PaddingByte = BR.ReadByte();
        if (PaddingByte != 0)
          Console.WriteLine("MonsterSpellInfo: Entry #{0}: Padding Byte #{1} is non-zero: {2:X2} ({2})", this.Index_, i + 1, PaddingByte);
      }
#endif
      BR.Close();
      return true;
