...
namespace PlayOnline.Core.Audio {
  ...
  public class AudioFileStream : Stream {
	...
	this.Codec_ = new ADPCMCodec(this.Header_.Channels, this.Header_.BlockSize);
      ...
      if (AddWAVHeader) { // Prepare WAV file header
	...
	return CookedPos;
      ...
      set {
	throw new NotSupportedException(I18N.GetText("SeekNotAllowed"));
      }
    }
	...
	if (BytesAvail >= count) {
	  ...
	  if (this.Buffer_ == null || this.BufferSize_ == this.Buffer_.Length) { // There's more data to be read from the file
	    this.FillBuffer();
	    goto ReadSomeMore;
	  }
	}
    ...
    public override void SetLength(long value) {
      throw new NotSupportedException(I18N.GetText("SetLengthNotAllowed"));
    }
